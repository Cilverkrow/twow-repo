...
import { APPROVED_SYSTEM_INSTRUCTION, loadRuntimeConfiguration, validateConfigObject, validateContextObject } from '../src/configuration.mjs';
import { acquireInstanceLock } from '../src/instance-lock.mjs';
import { strictLines } from '../src/ndjson.mjs';
import { preparePrompt, sanitizeAssistantText } from '../src/prompt.mjs';
...
test('pinned runtime config and versioned file context validate and freeze', () => {
  ...
  assert.equal(runtime.config.max_output_characters, 240);
  assert.equal(runtime.config.max_output_utf8_bytes, 240);
  assert.equal(runtime.config.max_output_sentences, 2);
  assert.equal(runtime.context.profile_version, 1);
  assert.equal(runtime.context.bot_guid, 18281);
  assert.equal(runtime.context_sha256, '386659245CB8298221465FD8B40339C13A01C7C10CBC58E876CDD264DC64D07E');
  assert.equal(runtime.context.race.name_en, 'Undead');
  assert.equal(runtime.context.class.name_en, 'Rogue');
  assert.equal(runtime.config.fixed_system_instruction, APPROVED_SYSTEM_INSTRUCTION);
  assert.match(runtime.context.dialogue_rule, /Always reply briefly in English/);
...
test('request schema rejects missing, unknown, wrong-version and unsafe integer fields', () => {
  ...
  const prototypeMember = parseJsonBytesStrict(Buffer.from('{"schema_version":1,"request_id":"11111111-1111-4111-8111-111111111111","bot_guid":18281,"created_utc":"2030-01-01T00:00:00.000Z","expires_utc":"2030-01-01T00:00:30.000Z","message":"Hello","__proto__":{"polluted":true}}'));
  assert.throws(
...
test('assembled prompt is byte-bounded after system, context and user expansion', () => {
  const request = requestContract(makeRequest(1, { message: 'Was hast du heute vor?' }));
  ...
  assert.equal(exact.messages[1].content, 'Was hast du heute vor?');
  assert.equal(exact.messages[0].role, 'system');
  assert.match(exact.messages[0].content, /Always reply briefly in English/);
  assert.match(exact.messages[0].content, /regardless of the language of the user's message/);
  assert.doesNotMatch(exact.messages[0].content, /Antworte|Deutsch|Persönlichkeitsdatei/);
  assert.ok(Object.isFrozen(exact.messages));
});

test('exactly two sentence-terminator runs are accepted without whitespace', () => {
  assert.equal(sanitizeAssistantText('One.Two!', runtime.config), 'One.Two!');
  assert.equal(sanitizeAssistantText('Really?!Yes...', runtime.config), 'Really?!Yes...');
});

test('three sentence-terminator runs without spaces are rejected', () => {
  assert.throws(
    () => sanitizeAssistantText('One.Two!Three?', runtime.config),
    (error) => error.code === 'too_many_sentences',
  );
});

test('Unicode ellipsis is enforced as a sentence terminator run', () => {
  assert.equal(sanitizeAssistantText('One…Two…', runtime.config), 'One…Two…');
  assert.throws(
    () => sanitizeAssistantText('One…Two…Three…', runtime.config),
    (error) => error.code === 'too_many_sentences',
...
test('assistant output permits at most 240 UTF-8 bytes without splitting a codepoint', () => {
  const asciiExact = 'a'.repeat(240);
  assert.equal(Buffer.byteLength(asciiExact, 'utf8'), 240);
  assert.equal(sanitizeAssistantText(asciiExact, runtime.config), asciiExact);
  assert.throws(
    () => sanitizeAssistantText('a'.repeat(241), runtime.config),
    (error) => error.code === 'assistant_text_too_long',
  );

  const multibyteExact = 'é'.repeat(120);
  assert.equal(Array.from(multibyteExact).length, 120);
  assert.equal(Buffer.byteLength(multibyteExact, 'utf8'), 240);
  assert.equal(sanitizeAssistantText(multibyteExact, runtime.config), multibyteExact);
  const multibyteOver = 'é'.repeat(121);
  assert.equal(Buffer.byteLength(multibyteOver, 'utf8'), 242);
  assert.throws(
    () => sanitizeAssistantText(multibyteOver, runtime.config),
    (error) => error.code === 'assistant_text_too_many_bytes',
  );
});

test('English-only system rule and output limits are immutable pins', () => {
  for (const mutation of [
    (config) => { config.max_output_characters = 241; },
    (config) => { config.max_output_utf8_bytes = 241; },
    (config) => { config.max_output_sentences = 3; },
    (config) => { config.fixed_system_instruction = `${config.fixed_system_instruction} Changed`; },
  ]) {
...
test('personality context byte tampering is detected by its pinned SHA-256', () => {
  ...
  try {
    ...
    const contextPath = resolve(contextDirectory, 'personality-context-profile-v1.json');
