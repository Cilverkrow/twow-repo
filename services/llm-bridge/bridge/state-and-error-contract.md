...
`created_utc` and `expires_utc` are preserved exactly as immutable wire evidence. On first admission only, the bridge snapshots wall time and monotonic time and derives one private deadline as `admission_monotonic + (expires_utc - admission_wall)`. At every later boundary, `monotonic_now >= admission_deadline` means expired. The same absolute deadline governs queued, running, ready-before-consume, connection, and response decisions. It is never exposed in a request, status, or completion envelope, never recomputed at run start, and never replaced by a duplicate. Forward or backward wall-clock changes after admission therefore cannot shorten or extend the accepted lifetime.

Text from an inference that completes late, or from a ready result that is not consumed before the monotonic deadline, is discarded and cannot be returned by `consume`.
...
- Final output permits at most `max_output_sentences` maximal contiguous terminator runs drawn from `.`, `!`, `?`, and `…`; a following space is not required, and a mixed run such as `?!` counts once.
- Final output is additionally limited to 240 UTF-8 bytes as a whole string. Over-limit text is rejected; it is never byte-truncated, so a multibyte UTF-8 codepoint cannot be split.
- System and personality rules require English dialogue regardless of input language. Output remains dialogue text only and cannot encode an action, emote, command, or alternate delivery channel.
