...
export class BoundedBridge {
  ...
  submitBytes(bytes) {
    ...
    const admissionWallMs = this.wallClock();
    const admissionMonotonicMs = this.monotonicClock();
    const request = createRequestEnvelope(parsed, {
      config: this.config,
      nowMs: admissionWallMs,
    });
    ...
    const deadlineMonotonicMs =
      admissionMonotonicMs + (Date.parse(request.expires_utc) - admissionWallMs);
    const queuedUtc = new Date(admissionWallMs).toISOString();
    const job = {
      request,
      prompt,
      deadlineMonotonicMs,
  ...
  consume(requestId, botGuid) {
    ...
    job.status = createStatusEnvelope({
      ...
      updatedUtc: new Date(this.wallClock()).toISOString(),
  ...
  async processJob(key, job) {
    if (this.deadlineReached(job)) {
      this.setTerminal(job, {
        outcome: 'expired',
        errorCode: 'expired_before_run',
        model: null,
        rawResponseBytes: null,
        wallNowMs: this.wallClock(),
      });
      return;
    }

    const startedWallMs = this.wallClock();
    job.state = 'running';
    job.startedUtc = new Date(startedWallMs).toISOString();
    job.attemptCount = 1;
    ...
    try {
      const result = await this.transport.infer({
        ...
        deadlineMonotonicMs: job.deadlineMonotonicMs,
      });
      ...
      const wallNowMs = this.wallClock();
  ...
  expireIfDue(job) {
    ...
    if (job.state === 'ready') {
      ...
      job.completion = createCompletionEnvelope({
        ...
        completedUtc: new Date(wallNowMs).toISOString(),
        text: null,
      ...
      job.status = createStatusEnvelope({
        ...
        state: 'expired',
        queuedUtc: job.queuedUtc,
        updatedUtc: new Date(wallNowMs).toISOString(),
        attemptCount: job.attemptCount,
        errorCode: 'expired_before_consume',
      });
      return true;
  ...
  deadlineReached(job) {
    return this.monotonicClock() >= job.deadlineMonotonicMs;
  }

  setTerminal(job, { outcome, errorCode, model, rawResponseBytes, text = null, wallNowMs }) {
    if (job.state === 'ready' || job.state === 'failed' || job.state === 'expired' || job.state === 'consumed') {
      this.staleResultsDiscarded += 1;
      return false;
    }
    job.state = outcome;
    const completedUtc = new Date(wallNowMs).toISOString();
    job.completion = createCompletionEnvelope({
      request: job.request,
      outcome,
      model,
