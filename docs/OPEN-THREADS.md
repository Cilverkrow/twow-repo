...
| OT-025 | WS-00, WS-10, WS-40, WS-50, WS-80 | Claude Code repository restructuring | A foundational refactor is planned after the current documentation handover. The persistent-roster source is now secured in a tested commit, but module boundaries, dependency direction, and the refactor commit sequence are not yet approved. | Use `docs/HANDOVER-CLAUDE-CODE.md`; verify the final pushed base, propose the target tree and migration plan first, then refactor only in an isolated branch/worktree through reviewable buildable commits. |
