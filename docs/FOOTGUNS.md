...
| ID | Footgun | Failure mode | Guardrail |
|---|---|---|---|
| FG-054 | Assuming the tested LLM adapter is integrated | The validated Phase B-R1 adapter now lives at `services/llm-bridge/client-cpp/`, promoted out of runbook source copies. It is still NOT built, NOT referenced by any CMakeLists and NOT attached; `modules/mod-playerbots` carries the earlier debug path. | Read `services/llm-bridge/README.md` first. Integrating it as written re-adds 325 lines to the vendored bot tree; `mod-bot-brain` does the same job through `RegisterAiContextAugmenter` with zero delta. |
