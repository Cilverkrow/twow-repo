...
| FG-008 | Treating runbook source copies as integrated code | Roster candidate source still exists only under `runbooks/.../source-copies`, not in main source. (The LLM half of this was promoted to `services/llm-bridge/` and no longer applies.) | Integrate through reviewed commits; never build production directly from a runbook directory or copy files blindly. |
...
| FG-087 | Merging a stacked PR by merging its parent first | A PR based on another PR's branch does not reach `main` when that parent merges. GitHub marks the child **MERGED** -- it did merge, into its base *branch* -- and closes it, so it disappears from the open list looking delivered while `main` never received the commit. Measured: `twow-core#75` (the profession-pool fix) showed MERGED with `main` provably lacking its content, one minute after `#74` merged. Nothing warns, and the PR cannot be reopened. | Merge a stack **child first, parent second**, or re-target each child at `main` before merging anything. After merging any stack, verify content rather than status: `git merge-base --is-ancestor <child-sha> origin/main`, or grep `main` for a symbol the child introduced. A green PR and a MERGED badge are not evidence that `main` changed. |
...
| FG-088 | Reading a build-flavour macro as an inventory of the fork's content | `MANGOSBOT_ZERO` selects the 1.12 build; it does not say what THIS fork ships. Upstream's `#ifndef MANGOSBOT_ZERO` guards encode assumptions about retail vanilla, and Turtle adds content vanilla never had. Measured: `SKILL_JEWELCRAFTING` (755) is guarded out of `PlayerbotFactory::tradeSkills[]` as a TBC profession, while the shipped world data carries a `'Jewelcrafting Trainer'` creature, eight `'Jewelcrafting Supplies'` vendors, five gossip options, items and points of interest -- and `ahbot/TradeCategory.cpp` already trades its recipes. Reasoning from the macro produced a confident, wrong review objection against a correct PR, and the same wrong guard had been copied into new code an hour earlier. | Decide content questions from the **shipped data** -- `sql/base/tw_world_*.sql`, DBC, trainers, gossip -- never from a build macro or from what retail 1.12 had. And read the issue text: OPS-005 (`docs/issues/22-runbook-sweep-ops.md:153`) already said "Jewelcrafting is player-available but never bot-selected", which is exactly right and was quoted in the same session that then contradicted it. |
...
| ID | Footgun | Failure mode | Guardrail |
|---|---|---|---|
| FG-054 | Assuming the tested LLM adapter is integrated | The validated Phase B-R1 adapter now lives at `services/llm-bridge/client-cpp/`, promoted out of runbook source copies. It is still NOT built, NOT referenced by any CMakeLists and NOT attached; `core/modules/mod-playerbots` carries the earlier debug path. | Read `services/llm-bridge/README.md` first. Integrating it as written re-adds 325 lines to the vendored bot tree; `mod-bot-brain` does the same job through `RegisterAiContextAugmenter` with zero delta. |
...
| FG-074 | Treating `TODOS.md` as permission | A checklist can be mistaken for an approved deployment or database task. | Every checklist remains non-authorizing; require an explicit mutation scope for execution. The historical workstream-hub preflight is retired by `AGENTS.md`. |
...
| FG-080 | Editing a generated runtime `.conf`, preserving it across renders, or appending overrides indefinitely | The runtime copy becomes an unreviewed source of truth; duplicate keys depend on parser order and a few bytes can change without attribution or a reproducible rollback point. | Change the complete template/reviewed semantic overlay in Git, require one final occurrence per canonical/connection key, render from a clean approved revision, and verify the secret-free provenance record. |
| FG-081 | A non-recursive checkout of this repository | The server core is the `core/` submodule and the core carries the Eluna engine as a submodule of *its* own. One level short and the build dies in the core's `FATAL_ERROR` about a missing Eluna; zero levels and it dies in the root one about an empty `core/`. Both read as a broken build rather than a shallow checkout. | Always `--recurse-submodules` / `submodules: recursive`. Both `FATAL_ERROR`s name the command to run. |
| FG-082 | Reading `issue-tracker-drift.sh`'s "tracker matches the manifests" as a two-way statement | It is one-way. The script walks the manifest entries and checks each one has an issue; it never walks the tracker and asks which issues no manifest entry explains. An issue filed by hand on GitHub -- bypassing the manifest that `docs/issues/README.md` calls the source of truth -- is therefore invisible to it, permanently, and a clean run says nothing about it. Two exist today: CORE-11 (#144) and CORE-12 (#145), 89 distinct ids on the tracker against 87 in the manifests. The header line hides rather than reveals the gap: its "issues with an id: 122" counts issues, including closed duplicates of the same id, so it never lines up with the manifest count and a reader stops expecting it to. | Until the reverse check exists, compare the two id sets yourself before trusting a clean run. Filing an issue by hand is the underlying defect: backfill it into `docs/issues/` and re-sync with `ops/issues/import-issues.sh --apply --update --only <ID>`. |
| FG-083 | Rebuilding before classifying the change | Script-only iterations pay for unchanged C++, while a cache hit is mistaken for proof that the configured build tree is compatible. | Write the `BUILD_PLAN` from `AGENTS.md`; select the smallest sufficient target and verify source, toolchain, flags, architecture, linkage, and install-prefix compatibility. Treat `ccache` and an incremental build tree as different assets. |
| FG-084 | Starting a replacement build before the abandoned one ended | A client can exit or be interrupted while its container and Ninja children continue consuming CPU, memory, and disk. Concurrent attempts then distort timing and capacity evidence. | Allow one expensive lane per task. Before replacement, verify the client and the task-owned container/Ninja process ended. Record interrupted/failed work as non-passing. |
| FG-085 | Detecting readiness from a fixed log tail | High-volume startup SQL output can push a valid ready marker outside the tail and create a false timeout or unnecessary restart. | Latch readiness from an observation started before the identified process/container; bind it to identity and start time. Use bounded tails only for diagnosis. |
| FG-086 | Treating fresh bootstrap and warm-state testing as interchangeable | Rebuilding every database wastes time, while reusing an incompatible or unapproved snapshot can hide migration defects or retain sensitive state. | Prove fresh bootstrap separately. Reuse only an isolated snapshot with verified source/schema/ledger/cache compatibility and explicit snapshot-retention approval. |

FG-083 through FG-086 were promoted from the sanitized findings of
[OPS-021](https://github.com/Cilverkrow/twow-repo/issues/177). They are workflow
guardrails, not a claim of measured performance improvement.
