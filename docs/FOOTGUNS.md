...
| FG-005 | Confusing `origin` and `upstream` | `origin` is the private project repository; `upstream` is the source baseline. An accidental push or merge can publish or reintroduce unwanted content. | Inspect remotes before fetch/push; never push private work to `upstream`. |
...
| FG-007 | Reintroducing binaries through an upstream merge | A normal merge can bring back binary blobs excluded from the published history. | Run binary-size/type and full-history secret scans before push. |
...
| FG-035 | Assuming TLS client defaults work locally | One verified Windows MariaDB client path failed Schannel negotiation before authentication. | Diagnose first; use a reviewed loopback-only client mode without changing server policy. |
...
| FG-075 | Bare ignore patterns that also match directories | `.gitignore`'s core-dump rule was the single word `core`, which matches the `core/` submodule directory as readily as a `core` dump file. `git submodule add core` was refused outright; forced past, `git status` would have reported nothing about the submodule ever again. The same line previously matched `deploy/docker/Dockerfile.core` under a `*.core` spelling and kept that Dockerfile out of the repository entirely. | Anchor and qualify: `core.[0-9]*` for the dumps, an explicit `!/core/` negation for the path. Before adding a path that could collide with an ignore rule, run `git check-ignore -v <path>` on it. |
| FG-076 | Assuming `.dockerignore` patterns are recursive | They are anchored at the context root. `sql/` kept 130 MB of world data out of the build context; the moment that data moved into the `core/` submodule the same files came back as `core/sql/`, matched by nothing, and the entry silently stopped doing its job. | Re-check every exclusion against the submodule tree as well as this one. Verify with `docker build --progress=plain` and read the transferred context size, not the pattern list. |
| FG-077 | Deleting a duplicated file because it "belongs to the core" | Seven files under `src/` that look like upstream duplicates carry fork bug fixes that exist only in this repository — a `strtok`→`strtok_r` fix on a path several map threads share, a missing `m_visibleGUIDs` lock, a virtual `~Spell()` the playerbots module depends on, a `sprintf` stack smash. Seventeen more headers carry self-containment includes the core lacks, four SQL files carry `INSERT IGNORE` repairs, and some of the C++ fixes were still uncommitted in the working tree when the submodule landed. They are duplicates by path, not by content. | Compare content, not paths, and normalise line endings first: a CRLF checkout makes every file look different and hides the handful that really are. Check `git status` too — an uncommitted fix is destroyed by the delete with no reflog to recover it. Nothing leaves this repository until the same change is committed in `twow-core` and the submodule pin advanced past it. |
