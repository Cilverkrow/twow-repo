...
the fork point in the repository; nothing else may restate it.
...
in `docs/history/source-commit-map.tsv`; that file is generated evidence and is the only
place a filtered-side hash for the fork point is written down.
...
- `docs/history/source-commit-map.tsv` (generated by `git-filter-repo`)
