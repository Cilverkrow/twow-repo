# Canonical Collaboration Hub Policy (REPOSITORY)

This file is the portable repository form of the local policy. The byte-identical local snapshot is preserved at `docs/history/AGENTS.local-snapshot.md`.

Paths in this policy are relative to the repository root. Historical runbooks may retain absolute `C:\TW\ComTW` paths as evidence; they do not authorize access to or mutation of that live workspace.
