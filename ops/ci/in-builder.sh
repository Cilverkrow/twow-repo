...
# ccache compression: level 6, not zstd's default-fast 1. The cost level 1
# avoids is on the STORE side only - zstd decompresses at essentially the
# same speed whatever level compressed it - and size is what matters here:
# the repository shares a 10 GB Actions cache budget, and a compiler cache
# that does not fit is evicted before the next run can restore it. That
