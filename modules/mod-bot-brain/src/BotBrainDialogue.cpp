...
namespace botbrain
{
    namespace
    {
        ...
        // The provider. WORKER THREAD ONLY. Returns the line to say, or "" for
        // silence -- and silence is the answer on every path but one.
        std::string Speak(BotDialogueRequest const& request)
        {
            ...
            if (!DecodeDialogueResponse(result.body, response, error))
            {
            ...
            if (!response.spoke)
            {
                ...
                // latched token budget or an open breaker.
