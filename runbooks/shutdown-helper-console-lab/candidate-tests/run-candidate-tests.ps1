...
$TestRoot = 'C:\TW\ComTW\runbooks\shutdown-helper-console-lab\candidate-tests'
...
function Invoke-HelperCandidate {
    ...
    try {
        ...
        & $PowerShell51 `
            ...
            -WorldWindowTitle $ExpectedTitle `
            -SaveDelaySeconds 1 `
            -WorldExitTimeoutSeconds $ExitTimeoutSeconds
...
function Test-PositiveShutdown {
    ...
    if (-not $visible.Contains('[OK] Worldserver PID')) {
        throw "The positive helper success message was not captured: $visible"
    }
