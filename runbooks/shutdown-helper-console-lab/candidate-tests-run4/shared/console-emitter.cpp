...
namespace
{
    ...
    int ParseExitCode(const char* value)
    {
        ...
        if (value == end || *end != '\0' || parsed < 0 || parsed > 255)
            throw std::runtime_error("The shutdown exit code must be an integer from 0 through 255.");
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (int index = 1; index < argc; ++index)
        {
            ...
            if (argument == "--shutdown-exit-code")
            {
                if (++index >= argc)
                    throw std::runtime_error("--shutdown-exit-code requires a value.");

                shutdownExitCode = ParseExitCode(argv[index]);
            }
            else if (argument == "--ignore-shutdown")
            {
                ignoreShutdown = true;
            }
            else
            {
                throw std::runtime_error("Unknown emitter argument: " + argument);
            }
        }
        ...
        ready << "PID=" << processId << "\n"
              ...
              << "IGNORE_SHUTDOWN=" << (ignoreShutdown ? "true" : "false") << "\n"
        ...
        while (std::getline(std::cin, line))
        {
            ...
            if (line == "server shutdown 0")
            {
                ...
                WriteResult(directory, shutdownExitCode, saveAllSeen, receivedLineCount,
