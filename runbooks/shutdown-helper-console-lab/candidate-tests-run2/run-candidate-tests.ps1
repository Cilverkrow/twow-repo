...
$TestRoot = 'C:\TW\ComTW\runbooks\shutdown-helper-console-lab\candidate-tests-run2'
...
function Invoke-HelperCandidate {
    ...
    try {
        ...
        $helperProcess = Start-Process -FilePath $PowerShell51 `
            ...
            -NoNewWindow `
            -PassThru
        $helperWaitMilliseconds = ($ExitTimeoutSeconds + 30) * 1000
        if (-not $helperProcess.WaitForExit($helperWaitMilliseconds)) {
            throw "Helper process PID $($helperProcess.Id) exceeded its test wait boundary."
        }
