...
function Write-StageDiagnostic {
    ...
    try {
        $diagnosticDirectory = [System.IO.Path]::GetFullPath($ServerDirectory)
        $diagnosticPath = Join-Path $diagnosticDirectory 'instrumented-helper-stages.log'
        ...
        [Console]::Error.WriteLine("[DIAGNOSTIC WRITE FAILURE] $($_.Exception.Message)")
    }
}
...
function Get-ExceptionDiagnostic {
    ...
    while ($null -ne $cursor) {
        if ($cursor -is [System.ComponentModel.Win32Exception]) {
            $nativeCode = [string]$cursor.NativeErrorCode
            $nativeMessage = $cursor.Message
            break
        }
        $cursor = $cursor.InnerException
    }

    $message = $ErrorRecord.Exception.Message.Replace("`r", '<CR>').Replace("`n", '<LF>')
    $nativeMessage = $nativeMessage.Replace("`r", '<CR>').Replace("`n", '<LF>')
    return 'TYPE={0}|MESSAGE={1}|WIN32_CODE={2}|WIN32_MESSAGE={3}' -f `
        $ErrorRecord.Exception.GetType().FullName, $message, $nativeCode, $nativeMessage
}

Write-StageDiagnostic 'SCRIPT_BEGIN'
...
function Invoke-InValidatedConsole {
    ...
    Write-StageDiagnostic "INITIAL_DETACH_BEFORE|SERVER_PID=$($ServerProcess.Id)"
    [TortoiseConsoleControl]::Detach()
    Write-StageDiagnostic "INITIAL_DETACH_AFTER|SERVER_PID=$($ServerProcess.Id)"
    $attached = $false
    try {
        Write-StageDiagnostic "ATTACH_CONSOLE_BEFORE|SERVER_PID=$($ServerProcess.Id)"
        [TortoiseConsoleControl]::AttachTo([uint32]$ServerProcess.Id)
        $attached = $true
        Write-StageDiagnostic "ATTACH_CONSOLE_AFTER|SERVER_PID=$($ServerProcess.Id)"

        $actualTitle = [TortoiseConsoleControl]::GetTitle()
        Write-StageDiagnostic "TITLE_VALIDATION_BEFORE|SERVER_PID=$($ServerProcess.Id)|ACTUAL=$actualTitle|EXPECTED=$ExpectedWindowTitle"
        ...
        Write-StageDiagnostic "TITLE_VALIDATION_AFTER|SERVER_PID=$($ServerProcess.Id)|RESULT=MATCH"
        ...
        if ($attached) {
            Write-StageDiagnostic "TARGET_DETACH_BEFORE|SERVER_PID=$($ServerProcess.Id)"
            [TortoiseConsoleControl]::Detach()
            Write-StageDiagnostic "TARGET_DETACH_AFTER|SERVER_PID=$($ServerProcess.Id)"
        }

        Write-StageDiagnostic "PARENT_ATTACH_BEFORE|SERVER_PID=$($ServerProcess.Id)"
        [TortoiseConsoleControl]::TryAttachToParent()
        Write-StageDiagnostic "PARENT_ATTACH_AFTER|SERVER_PID=$($ServerProcess.Id)"
    }
}
...
try {
    ...
    if ($Action -eq 'World') {
        ...
        Write-StageDiagnostic "WORLD_PID_VALIDATION_BEFORE|EXE=$worldExecutable|PID_FILE=$worldPidFile"
        $worldProcess = Get-ValidatedServerProcess `
            -ProcessName 'mangosd' `
            -ExpectedExecutable $worldExecutable `
            -PidFile $worldPidFile
        Write-StageDiagnostic "WORLD_PID_VALIDATION_AFTER|PID=$($worldProcess.Id)|PATH=$($worldProcess.Path)"

        $validatedTitle = Invoke-InValidatedConsole `
            -ServerProcess $worldProcess `
            -ExpectedWindowTitle $WorldWindowTitle `
            -Operation {
                Write-StageDiagnostic "WRITECONSOLEINPUT_SAVEALL_BEFORE|SERVER_PID=$($worldProcess.Id)"
                [TortoiseConsoleControl]::WriteCommand('saveall')
                Write-StageDiagnostic "WRITECONSOLEINPUT_SAVEALL_AFTER|SERVER_PID=$($worldProcess.Id)"
                ...
                Write-StageDiagnostic "WRITECONSOLEINPUT_SHUTDOWN_BEFORE|SERVER_PID=$($worldProcess.Id)"
                [TortoiseConsoleControl]::WriteCommand('server shutdown 0')
                Write-StageDiagnostic "WRITECONSOLEINPUT_SHUTDOWN_AFTER|SERVER_PID=$($worldProcess.Id)"
            }

        Write-StageDiagnostic "WORLD_WAIT_BEFORE|PID=$($worldProcess.Id)|TIMEOUT_SECONDS=$WorldExitTimeoutSeconds"
        $worldWaitResult = $worldProcess.WaitForExit($WorldExitTimeoutSeconds * 1000)
        Write-StageDiagnostic "WORLD_WAIT_AFTER|PID=$($worldProcess.Id)|RESULT=$worldWaitResult"
        if (-not $worldWaitResult) {
            throw "mangosd.exe laeuft nach $WorldExitTimeoutSeconds Sekunden noch. Es wird kein erzwungener Shutdown ausgefuehrt."
        }

        $worldProcess.Refresh()
        $exitCodeProperty = $worldProcess.PSObject.Properties['ExitCode']
        $propertyMemberType = if ($null -eq $exitCodeProperty) { '<missing>' } else { [string]$exitCodeProperty.MemberType }
        $propertyTypeName = if ($null -eq $exitCodeProperty) { '<missing>' } else { [string]$exitCodeProperty.TypeNameOfValue }
        $observedExitCode = $worldProcess.ExitCode
        $observedExitCodeText = if ($null -eq $observedExitCode) { '<null>' } else { [string]$observedExitCode }
        $clrExitCodeText = '<not-read>'
        $clrExitCodeError = '<none>'
        try {
            $clrExitCode = $worldProcess.GetType().GetProperty('ExitCode').GetValue($worldProcess, $null)
            $clrExitCodeText = if ($null -eq $clrExitCode) { '<null>' } else { [string]$clrExitCode }
        }
        catch {
            $clrExitCodeError = $_.Exception.Message.Replace("`r", '<CR>').Replace("`n", '<LF>')
        }
        Write-StageDiagnostic "WORLD_EXIT_CODE_CHECK_BEFORE|PID=$($worldProcess.Id)|HAS_EXITED=$($worldProcess.HasExited)|ETS_MEMBER_TYPE=$propertyMemberType|ETS_TYPE=$propertyTypeName|ETS_VALUE=$observedExitCodeText|CLR_VALUE=$clrExitCodeText|CLR_ERROR=$clrExitCodeError"
        if ($observedExitCode -ne 0) {
            throw "mangosd.exe wurde mit dem unerwarteten Exitcode $observedExitCode beendet."
        }
        Write-StageDiagnostic "WORLD_EXIT_CODE_CHECK_AFTER|PID=$($worldProcess.Id)|RESULT=ZERO"
    ...
    $diagnostic = Get-ExceptionDiagnostic -ErrorRecord $_
    Write-StageDiagnostic "FAILURE|$diagnostic"
