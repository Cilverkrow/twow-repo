...
$TestRoot = 'C:\TW\ComTW\runbooks\shutdown-helper-console-lab\title-compatibility-realm-final'
...
public static class RealmLabNative
{
    ...
    public static string ReadConsoleSnapshot()
    {
        IntPtr output = GetOutputHandle();
        CONSOLE_SCREEN_BUFFER_INFO info = GetInfo(output);
        long count = ((long)info.CursorPosition.Y + 1) * info.Size.X;
        if (count == 0)
            return String.Empty;
        if (count > 1048576)
            throw new InvalidOperationException("The console snapshot exceeded one MiB.");
        ...
        if (!ReadConsoleOutputCharacterW(output, buffer, (uint)count, origin, out read))
            throw new Win32Exception(Marshal.GetLastWin32Error(), "ReadConsoleOutputCharacterW failed");
        if (read != (uint)count)
            throw new InvalidOperationException("Only part of the console snapshot was captured.");
        return buffer.ToString();
    }
...
function Invoke-HelperCandidate {
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($visible)) {
            $visible = [RealmLabNative]::ReadConsoleSnapshot()
        }
    }
    finally {
        $timer.Stop()
...
function Test-RealmSuccess {
    ...
    $null = Start-ValidatedEmitter `
        ...
        -Arguments '--title "Administrator:  realmd" --ctrl-break-exit-code 0 --idle-exit-ms 15000' `
        -ExpectedTitle 'Administrator:  realmd'
