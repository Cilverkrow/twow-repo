...
$TestRoot = 'C:\TW\ComTW\runbooks\shutdown-helper-console-lab\title-compatibility-world-final'
...
$ExpectedHelperCandidateSha256 = '76D899BE55BAE77E72CCD5DF6C5CBD8203986524E944C3AEE7B8C2DD7862EA1A'
...
function Invoke-HelperCandidate {
    ...
    try {
        ...
        if ($null -eq $helperProcess) {
            throw 'The helper process could not be created.'
        }
...
function Test-PositiveShutdown {
    ...
    $launcher = Join-Path $scenario 'launch-elevated-title.bat'
    New-LabLauncher -Path $launcher -Lines @(
        '@echo off',
        'title Administrator:  mangosd',
        'cd /d "%~dp0"',
        'mangosd.exe'
    )

    $emitter = Start-ValidatedEmitter -ScenarioDirectory $scenario -LauncherPath $launcher -ExpectedTitle 'Administrator:  mangosd'
    $ready = Get-ReadyValues -Path (Join-Path $scenario 'emitter-ready.txt')
    if ($ready['TITLE'].Length -ne 23) {
        throw "Disposable elevated title length is $($ready['TITLE'].Length), expected 23."
    }
    ...
    Write-LabEvent "TEST_PASS|POSITIVE|HELPER_EXIT=0|EMITTER_EXIT=0|TITLE=Administrator:_two_spaces_mangosd|COMMANDS=saveall,server shutdown 0"
}
...
function Test-WrongTitle {
    ...
    $liveEmitter = Get-Process -Id $script:CurrentEmitterPid -ErrorAction Stop
    $liveEmitter.Refresh()
    if ($liveEmitter.HasExited) {
        throw 'The helper terminated the wrong-title emitter.'
    }
...
function Test-PidMismatch {
    ...
    $liveEmitter.Refresh()
    if ($liveEmitter.HasExited) {
        throw 'The helper terminated the PID-mismatch emitter.'
    }
