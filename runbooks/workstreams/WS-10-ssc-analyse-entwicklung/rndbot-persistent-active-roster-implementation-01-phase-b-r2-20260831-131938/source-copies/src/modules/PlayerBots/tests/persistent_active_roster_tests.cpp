...
struct FakeData
{
    ...
    std::function<RuntimeState()> stateProbe;
    std::vector<RuntimeState> observedLoadStates;
};
...
public:
    ...
    {
        if (data_->stateProbe) data_->observedLoadStates.push_back(data_->stateProbe());
        if (data_->schemaValid) return true;
    ...
    {
        replayed = false;
        auto existing = data_->operations.find(request.operationId);
        if (existing != data_->operations.end())
        {
            if (existing->second.requestSha256 != requestSha256)
            {
                error = "OPERATION_ID_REQUEST_MISMATCH";
                return false;
            }
            replayed = true;
            replayRecord = existing->second;
            return true;
        }
        if (data_->hasCurrent != hasBefore || (hasBefore && data_->current.versionId != before.versionId))
        {
            error = "CURRENT_VERSION_MISMATCH";
            return false;
        }
        if (data_->failCommit) { error = "INJECTED_TRANSACTION_FAILURE"; return false; }
        after.versionId = data_->hasCurrent ? data_->current.versionId + 1 : 1;
...
{
    std::ifstream input(std::string(ROSTER_TEST_FIXTURE_DIR) + "/" + name, std::ios::binary);
    CHECK(input.good());
...
void TestCanonicalVectors()
{
    ...
    CHECK(empty.size() == 68);
    CHECK(ReadFixture("empty-snapshot-v1.txt") == empty);
    CHECK(Hex(Hash(empty)) == "BA46C4A526EE8BBE3A640492A1167DE0A449D382FE129891BF38BA89E3DF293E");
    Sha256 parsedLower{}, parsedUpper{};
    CHECK(ParseHex("ba46c4a526ee8bbe3a640492a1167de0a449d382fe129891bf38ba89e3df293e", parsedLower));
    CHECK(ParseHex("BA46C4A526EE8BBE3A640492A1167DE0A449D382FE129891BF38BA89E3DF293E", parsedUpper));
    CHECK(parsedLower == parsedUpper);

    AdminRequest source = MakeInitialize({1, 42, 4294967295u});
    std::string canonical = Wire(source);
    CHECK(ReadFixture("initialize-request-v1.txt") == canonical);
...
void TestStartupStates()
{
    ...
    auto badSchemaData = std::make_shared<FakeData>(); badSchemaData->schemaValid = false;
    Service badSchema(std::unique_ptr<Store>(new FakeStore(badSchemaData)));
    CHECK(!badSchema.Start(true, false, error));
    CHECK(badSchema.State() == RuntimeState::INVALID_FAIL_CLOSED);
    CHECK(error == "SCHEMA_FINGERPRINT_MISMATCH");
    CHECK(badSchema.Apply(Wire(MakeInitialize({1}))).code == "STORE_NOT_VALIDATED");
    ...
    data->stateProbe = [&healthy]() { return healthy.State(); };
    CHECK(healthy.Start(true, false, error));
    CHECK(data->observedLoadStates.size() == 2);
    CHECK(data->observedLoadStates[0] == RuntimeState::LOADING && data->observedLoadStates[1] == RuntimeState::LOADING);
    CHECK(healthy.State() == RuntimeState::STARTING);
    CHECK(healthy.IsMember(10));
    CHECK(healthy.Status().rosterTarget == 2 && healthy.Status().rosterAvailable == 0 && healthy.Status().rosterOnline == 0);
    healthy.RecordAvailable(10);
    CHECK(healthy.Status().rosterAvailable == 1);
    ...
    healthy.RecordPending(20);
    CHECK(healthy.Status().rosterOnline == 1);
    CHECK(healthy.State() == RuntimeState::DEGRADED);
    CHECK(healthy.Status().diagnostics.size() == 1 && healthy.Status().diagnostics.front().first == 20);
    healthy.Stop();
...
void TestOperationsAndRestart()
{
    ...
    CHECK(first.accepted && !first.replayed && first.restartRequired && first.versionId == 1);
    CHECK(first.code == "APPLIED_RESTART_REQUIRED");
    ...
    CHECK(expanded.accepted && expanded.restartRequired && expanded.versionId == 2);
    CHECK(restart.State() == RuntimeState::INVALID_FAIL_CLOSED);
    CHECK(restart.Desired() == std::vector<uint32_t>({101, 202, 303}));
    CHECK(data->current.guids == std::vector<uint32_t>({101, 202, 303, 404, 505}));
    CHECK(std::equal(data->versions[1].guids.begin(), data->versions[1].guids.end(), data->versions[2].guids.begin()));

    Service afterExpand(std::unique_ptr<Store>(new FakeStore(data)));
    CHECK(afterExpand.Start(true, false, error));
    AdminRequest remove;
    ...
    CHECK(data->current.guids == std::vector<uint32_t>({101, 303, 404, 505}));

    Service afterRemove(std::unique_ptr<Store>(new FakeStore(data)));
    CHECK(afterRemove.Start(true, false, error));
    ...
    CHECK(data->current.guids == std::vector<uint32_t>({111, 303, 404, 505}));

    Service afterReplace(std::unique_ptr<Store>(new FakeStore(data)));
    CHECK(afterReplace.Start(true, false, error));
    ...
    CHECK(afterReplace.Apply(Wire(rollback)).accepted);
    CHECK(data->current.guids == std::vector<uint32_t>({101, 202, 303}));
    ...
    CHECK(replaceData->current.guids == std::vector<uint32_t>({600, 200, 300}));
    ...
    CHECK(scaleData->current.guids.size() == 100);
    CHECK(std::equal(fifty.begin(), fifty.end(), scaleData->current.guids.begin()));
}
...
void TestFailureAndPolicy()
{
    ...
    data->failCommit = false;
    AdminRequest removeAll;
    removeAll.operationId="12121212-1212-4121-8121-121212121212"; removeAll.operationType=OperationType::REMOVE;
    removeAll.hasExpectedCurrentVersionId=true; removeAll.expectedCurrentVersionId=1; removeAll.actor="test";
    removeAll.reason="empty forbidden"; removeAll.remove={700,800}; removeAll.requestedTargetCount=0;
    ApplyResult emptyRejected = service.Apply(Wire(removeAll));
    CHECK(!emptyRejected.accepted && emptyRejected.code == "EMPTY_ROSTER_FORBIDDEN");
    CHECK(data->current.guids == before.guids);
    ...
    Service concurrentA(std::unique_ptr<Store>(new FakeStore(data)));
    Service concurrentB(std::unique_ptr<Store>(new FakeStore(data)));
    CHECK(concurrentA.Start(true, false, error)); CHECK(concurrentB.Start(true, false, error));
    AdminRequest opA = add; opA.operationId="13131313-1313-4131-8131-131313131313"; opA.add={901};
    AdminRequest opB = add; opB.operationId="14141414-1414-4141-8141-141414141414"; opB.add={902};
    CHECK(concurrentA.Apply(Wire(opA)).accepted);
    ApplyResult lostRace = concurrentB.Apply(Wire(opB));
    CHECK(!lostRace.accepted && lostRace.code == "CURRENT_VERSION_MISMATCH");
}
...
void TestR2Policies()
{
    CHECK(IsRosterAdminTransportAllowed(AdminTransport::LOCAL_CONSOLE));
    CHECK(!IsRosterAdminTransportAllowed(AdminTransport::REMOTE_ADMIN));
    CHECK(!IsRosterAdminTransportAllowed(AdminTransport::GAME_CHAT));
    CHECK(EvaluateRosterApplyGate(false, false, false, 0, false) == "ROSTER_DISABLED");
    CHECK(EvaluateRosterApplyGate(true, true, false, 0, false) == "MAINTENANCE_MODE_REQUIRED");
    CHECK(EvaluateRosterApplyGate(true, true, true, 1, false) == "ROSTER_BOTS_ONLINE");
    CHECK(EvaluateRosterApplyGate(true, true, true, 0, true) == "ROSTER_BOTS_ONLINE");
    CHECK(EvaluateRosterApplyGate(true, true, true, 0, false).empty());

    CHECK(EvaluateRosterAccountAdmission(false, false, false, false, false, false, 0, false) == "CHARACTER_NOT_FOUND");
    CHECK(EvaluateRosterAccountAdmission(true, false, false, false, false, false, 0, false) == "ACCOUNT_NOT_FOUND");
    CHECK(EvaluateRosterAccountAdmission(true, true, false, false, true, true, 0, false) == "ACCOUNT_NOT_RNDBOT_STOCK");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, true, true, true, 0, false) == "ACCOUNT_BANNED");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, false, false, false, 0, false) == "ACCOUNT_LOGIN_RECORD_NOT_FOUND");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, false, true, false, 0, false) == "ACCOUNT_INACTIVE");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, false, true, true, 1, false) == "ACCOUNT_LOCKED");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, false, true, true, 0, true) == "ACCOUNT_HAS_REGISTERED_SESSION");
    CHECK(EvaluateRosterAccountAdmission(true, true, true, false, true, true, 0, false).empty());

    RuntimeBehaviorPolicy roster = EvaluateRuntimeBehaviorPolicy(true, true);
    CHECK(roster.clearExpiredValues && roster.normalAiTicks && roster.normalStrategyMaintenance);
    CHECK(roster.travelAndIdleBehavior && roster.revive && roster.sessionMaintenance && roster.scheduleNextUpdate);
    CHECK(!roster.leaseLogout && !roster.populationRotation && !roster.randomizeProgression);
    CHECK(!roster.randomStrategyReassignment && !roster.randomTeleport && !roster.automaticGroupRemoval);
    RuntimeBehaviorPolicy legacy = EvaluateRuntimeBehaviorPolicy(false, false);
    CHECK(legacy.leaseLogout && legacy.populationRotation && legacy.randomizeProgression && legacy.randomTeleport);
}

void TestAdminRequestFileGate()
{
    namespace fs = std::filesystem;
    std::string bytes;
    std::string error;
    CHECK(!ReadCanonicalAdminRequestFile("relative-request.txt", bytes, error));
    CHECK(error == "ABSOLUTE_REQUEST_PATH_REQUIRED");
    fs::path fixture = fs::absolute(fs::path(ROSTER_TEST_FIXTURE_DIR) / "initialize-request-v1.txt");
    error.clear();
    CHECK(ReadCanonicalAdminRequestFile(fixture.string(), bytes, error));
    CHECK(bytes == ReadFixture("initialize-request-v1.txt"));
    AdminRequest parsed;
    CHECK(ParseAdminRequest(bytes, parsed, error));
    error.clear();
    CHECK(!ReadCanonicalAdminRequestFile(fixture.parent_path().string(), bytes, error));
    CHECK(error == "REQUEST_PATH_NOT_REGULAR_FILE");
}
}
