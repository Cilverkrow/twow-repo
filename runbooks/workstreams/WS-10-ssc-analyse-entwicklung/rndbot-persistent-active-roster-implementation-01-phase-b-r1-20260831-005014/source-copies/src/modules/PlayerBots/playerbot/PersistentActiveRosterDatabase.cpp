...
{
    std::unique_ptr<QueryResult> result(CharacterDatabase.Query(SchemaFingerprintQuery));
    if (!result)
    {
...
bool CharacterDatabaseStore::LoadMembers(Snapshot& snapshot, uint32_t memberCount, std::string& error)
{
    ...
    std::unique_ptr<QueryResult> members(CharacterDatabase.PQuery(
...
{
    found = false;
    std::unique_ptr<QueryResult> current(CharacterDatabase.Query(
    ...
    std::string const& resultCode, bool& replayed, OperationRecord& replayRecord, std::string& error)
{
    ...
    bool committed = CharacterDatabase.DirectTransaction([&](SqlConnection& connection) -> bool
    {
