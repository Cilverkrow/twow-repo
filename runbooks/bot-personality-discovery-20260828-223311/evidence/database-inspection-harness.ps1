...
function Assert-ReadOnlyQuerySet {
    ...
    foreach ($query in $Queries) {
        ...
        if ($text -notmatch '^(?is)SHOW\s+CREATE\s+TABLE\b' -and
            $text -match '(?is)\b(INSERT|UPDATE|DELETE|REPLACE|ALTER|DROP|TRUNCATE|RENAME|LOAD|CALL|LOCK|GRANT|REVOKE|CREATE\s+(TEMPORARY\s+)?TABLE)\b') {
            $writeCapable.Add("$($query.Id): contains a forbidden SQL token")
        }
    }
    $audit = [ordered]@{
