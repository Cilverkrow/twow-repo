...
const GUARD_PATH = resolve(EVIDENCE_ROOT, 'v1-english-one-shot.guard.json');
...
try {
  ...
  const englishVerification = verifyEnglishDialogue(firstConsume.value.completion.text, runtime.config);
  ...
  writeJsonExclusive('english-language-verification.json', {
    schema_version: 1,
    result: 'ENGLISH_OUTPUT_VERIFICATION=PASS',
    input_language: 'German',
    required_output_language: 'English',
    input_text: request.message,
    output_text: firstConsume.value.completion.text,
    ...englishVerification,
  });
  ...
  const result = {
    schema_version: 1,
    result: 'V1_ENGLISH_LIVE_INFERENCE=PASS',
    ...
    input_language: 'German',
    required_output_language: 'English',
    english_output_verified: true,
    sanitized_text_codepoints: englishVerification.codepoints,
    sanitized_text_utf8_bytes: englishVerification.utf8_bytes,
  ...
  if (!finalResultWritten) {
    writeJsonExclusive('live-run-result.json', {
      schema_version: 1,
      result: 'V1_ENGLISH_LIVE_INFERENCE=FAIL',
      started_utc: runStartedUtc,
      completed_utc: new Date().toISOString(),
      failure,
  ...
  process.stderr.write(`V1_ENGLISH_LIVE_INFERENCE=FAIL ${failure.message}\n`);
  process.exitCode = 1;
} finally {
...
function createOneShotGuard() {
  ...
  try {
    writeFileSync(fileDescriptor, `${JSON.stringify({
      schema_version: 1,
      purpose: 'prevents any restart, resubmit, or retry of the approved V1 English correction live run',
...
function verifyEnglishDialogue(text, config) {
  const codepoints = Array.from(text).length;
  const utf8Bytes = Buffer.byteLength(text, 'utf8');
  requireCondition(codepoints <= config.max_output_characters, `completion contains ${codepoints} codepoints`);
  requireCondition(utf8Bytes <= config.max_output_utf8_bytes, `completion contains ${utf8Bytes} UTF-8 bytes`);
  requireCondition(/^I(?:\b|['’])/u.test(text), 'completion does not begin as a first-person English sentence');
  requireCondition(
    /\b(?:I|am|will|plan|intend|today|going|stay|keep|watch|patrol|explore|rest|travel|help|visit|work)\b/iu.test(text),
    'completion lacks an English first-person dialogue marker',
  );
  requireCondition(
    !/\b(?:ich|heute|werde|möchte|plane|bleibe|gehe|halte|augen|offen|vorhaben)\b/iu.test(text),
    'completion contains a German dialogue marker',
  );
  requireCondition(!/^[*/\[]/u.test(text), 'completion begins with an action, emote, or command marker');
  requireCondition(!/\b(?:ollama|database|guid|language model|artificial intelligence)\b/iu.test(text), 'completion exposes a forbidden implementation term');
  return {
    codepoints,
    utf8_bytes: utf8Bytes,
    within_240_codepoints: true,
    within_240_utf8_bytes: true,
    first_person_english_marker: true,
    german_dialogue_marker_absent: true,
    action_emote_command_prefix_absent: true,
  };
}
