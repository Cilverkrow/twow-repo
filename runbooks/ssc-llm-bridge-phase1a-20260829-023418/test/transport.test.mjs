...
function transportInput(sequence = 1) {
...
test('model inventory requires exact pinned name and digest', async (t) => {
  await t.test('exact pin', async () => {
    await withServer((request, response) => {
      ...
      const transport = new OllamaTransport(transportConfig(baseUrl), () => FIXED_NOW_MS);
...
test('oversized raw inference response is rejected before JSON parsing', async () => {
  await withServer((request, response) => {
    ...
    const transport = new OllamaTransport(transportConfig(baseUrl, { max_raw_response_bytes: 1024 }), () => FIXED_NOW_MS);
    await assert.rejects(
