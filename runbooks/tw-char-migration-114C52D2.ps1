...
$ExpectedCanonicalScriptByteCount = 68357
...
function Invoke-MariaDb {
    ...
    $output = & $MariaDbClient @arguments 2>&1
    $exitCode = $LASTEXITCODE
    if ($exitCode -ne 0) {
        throw "MariaDB client failed with exit code $exitCode. Output: $($output -join ' ')"
    }
...
WHERE table_schema=DATABASE()
  ...
  AND is_nullable='NO'
  AND CAST(column_default AS CHAR)=''
  AND collation_name='utf8_general_ci'
  AND ordinal_position=3
'@
    Assert-SingleValue -Sql $sql -Expected '1' -Description 'migrations.Module exact definition'
}
...
function Get-AclManifestLines {
    ...
    foreach ($entry in @($entries | Sort-Object FullName)) {
        ...
        "{0}|{1}|{2}" -f $type, $relative, (Get-Acl -LiteralPath $entry.FullName).Sddl
    }
}
...
function Assert-NoReparsePoints {
    param([string]$Directory)
    $points = @(Get-ChildItem -LiteralPath $Directory -Recurse -Force |
        Where-Object { ($_.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 })
    if ($points.Count -gt 0) { throw "Reparse points were found below $Directory." }
}
...
function Copy-RootAcl {
    param([string]$Source, [string]$Destination)
    $sourceAcl = Get-Acl -LiteralPath $Source -ErrorAction Stop
    Set-Acl -LiteralPath $Destination -AclObject $sourceAcl -ErrorAction Stop
    $sourceRoot = @((Get-AclManifestLines -Directory $Source) | Select-Object -First 1)
    $destinationRoot = @((Get-AclManifestLines -Directory $Destination) | Select-Object -First 1)
    if ($sourceRoot.Count -ne 1 -or $destinationRoot.Count -ne 1 -or $sourceRoot[0] -cne $destinationRoot[0]) {
        throw "Root ACL preservation failed between '$Source' and '$Destination'."
    }
}
...
function Wait-ForDatabaseReady {
    ...
    throw 'MariaDB did not become ready on the verified port within 45 seconds.'
}
...
function Invoke-ExecuteMode {
    ...
    Assert-AclManifestMatchesDirectory -ManifestPath $sourceAclManifest -Directory $coldBackup
...
function Invoke-RollbackMode {
    ...
    Copy-RootAcl -Source $coldBackup -Destination $staging
    Assert-ManifestMatchesDirectory -ManifestPath $backupManifest -Directory $staging
    Assert-AclManifestMatchesDirectory -ManifestPath $backupAclManifest -Directory $staging
    ...
    Assert-AclManifestMatchesDirectory -ManifestPath $backupAclManifest -Directory $DataDir
    Assert-RestoredDatabaseConfiguration
    Assert-DatabaseProgramFiles
