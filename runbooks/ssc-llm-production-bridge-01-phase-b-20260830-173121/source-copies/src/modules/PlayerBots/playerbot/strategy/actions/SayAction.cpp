...
void ChatReplyAction::ChatReplyDo(Player* bot, uint32 type, uint32 guid1, uint32 guid2, std::string msg, std::string chanName, std::string name)
{
    ...
    // External v1 is intentionally narrower than the historical LLM path:
    // one pinned bot, real-player whispers only, and value-only admission.
    // A local reject (including the permanent ledger latch) deliberately
    // falls through to the ordinary Playerbot reply below.
    if (sPlayerbotAIConfig.externalLLMBridgeEnabled && chatChannelSource == ChatChannelSource::SRC_WHISPER &&
        bot && bot->GetGUIDLow() == ExternalLLMBridgeService::BotGuid)
    {
        Player* player = sObjectAccessor.FindPlayer(ObjectGuid(HIGHGUID_PLAYER, guid1));
        if (player && player != bot && IsRealPlayer(player) && player->IsInWorld() && player->GetSession() &&
            !player->GetSession()->isLogingOut() && player->GetName() == name)
        {
            ExternalLLMBridgeService::SubmitResult const result = ExternalLLMBridgeService::Instance().TrySubmit(
                bot->GetObjectGuid().GetRawValue(), player->GetObjectGuid().GetRawValue(), player->GetName(), msg);
            if (result == ExternalLLMBridgeService::SubmitResult::Admitted)
                return;
        }
    }
