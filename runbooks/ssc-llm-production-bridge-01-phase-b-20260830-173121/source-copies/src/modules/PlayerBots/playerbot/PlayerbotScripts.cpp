...
class PlayerbotWorldScript : public WorldScript
{
    public:
        ...
        void OnStartup() override
        {
            ...
            auctionbot.Init();
            ExternalLLMBridgeService::Instance().Start();
        }
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            auctionbot.Update();
            ExternalLLMBridgeService::Instance().UpdateWorld();
        }

        void OnShutdown() override
        {
            ExternalLLMBridgeService::Instance().Shutdown();
        }
};
...
class PlayerbotPlayerScript : public PlayerScript
{
    public:
        ...
        void OnLogin(Player* player) override
        {
            ...
            ExternalLLMBridgeService::Instance().ObserveSession(
                player->GetObjectGuid().GetRawValue(), player->GetSession()->GetAccountId(),
                static_cast<std::int64_t>(player->GetLoginTime()), player->GetName());
        ...
        {
            if (player)
                ExternalLLMBridgeService::Instance().InvalidateSession(player->GetObjectGuid().GetRawValue());
            RemoveBotAI(player);
            RemoveBotMgr(player);
        }
