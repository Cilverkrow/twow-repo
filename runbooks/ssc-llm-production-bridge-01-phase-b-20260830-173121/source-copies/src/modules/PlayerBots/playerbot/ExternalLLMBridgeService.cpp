...
{
    if (!ExactFields(value, { "schema_version", "request_id", "bot_guid", "state", "queued_utc", "updated_utc", "attempt_count", "error_code" }) ||
    ...
    else if (state == "expired")
    {
        ...
        if (error != "expired_before_run" && error != "stale_result" && error != "expired_before_consume") return false;
    }
    else if (state == "consumed")
    {
        if (!value["error_code"].is_null() && !KnownTerminalError(value["error_code"])) return false;
    }
    else
        return false;
...
{
    if (!ExactFields(value, { "schema_version", "request_id", "bot_guid", "outcome", "model", "attempt_count", "started_utc", "completed_utc", "text", "error_code", "raw_response_bytes" }) ||
    ...
    if (attempt == 0)
    {
        if (!value["started_utc"].is_null() || !value["model"].is_null() || !value["raw_response_bytes"].is_null())
            return false;
        if (error != "queue_full" && error != "expired_before_run" && error != "shutdown_cancelled" && error != "shutdown_timeout")
            return false;
    }
    else
    {
...
struct Route
{
    enum class Phase { PendingSubmit, Accepted, Terminal, Consuming };
...
bool ReadAvailable(HANDLE handle, std::string& output, bool& eof)
{
    eof = false;
    for (;;)
    {
        ...
        DWORD read = 0;
        if (!ReadFile(handle, buffer.data(), std::min<DWORD>(available, static_cast<DWORD>(buffer.size())), &read, nullptr))
        {
            DWORD const error = GetLastError();
            eof = error == ERROR_BROKEN_PIPE || error == ERROR_PIPE_NOT_CONNECTED;
            return eof;
...
bool WriteAll(HANDLE handle, std::string const& data)
{
    std::size_t offset = 0;
    while (offset < data.size())
    {
        DWORD written = 0;
        DWORD const amount = static_cast<DWORD>(std::min<std::size_t>(data.size() - offset, 16384));
...
bool VerifyPackage(std::string const& rootText, std::filesystem::path& cliPath, std::string& error)
{
    try
    {
        std::filesystem::path root = std::filesystem::u8path(rootText);
        if (!root.is_absolute()) { error = "package_root_not_absolute"; return false; }
        ...
        std::string digest;
        if (!Sha256File(manifest, digest) || digest != kPayloadManifestHash) { error = "payload_manifest_pin_mismatch"; return false; }
        ...
        while (std::getline(input, line))
        {
            ...
            std::filesystem::path file = std::filesystem::weakly_canonical(bridge / std::filesystem::u8path(relative));
        ...
        if (!pin(bridge / "config" / "bridge-config-v1.json", kConfigHash)) { error = "config_pin_mismatch"; return false; }
        if (!pin(bridge / "context" / "personality-context-profile-v1.json", kPersonalityHash)) { error = "personality_pin_mismatch"; return false; }
        cliPath = bridge / "src" / "cli.mjs";
        if (!pin(cliPath, kCliHash)) { error = "cli_pin_mismatch"; return false; }
        return true;
    }
    catch (...)
    {
...
public:
    ...
    SubmitResult TrySubmit(std::uint64_t botGuid, std::uint64_t targetGuid, std::string const& targetName, std::string const& message)
    {
        ...
        if (state != State::Ready || botGuid == 0 || targetGuid == 0 || botGuid == targetGuid || targetName.empty() ||
            message.empty() || message.size() > 2048) return SubmitResult::LocallyRejected;
    ...
    bool StartProcess(std::wstring const& executable, std::vector<std::wstring> const& arguments, std::uint32_t readyTimeout)
    {
        ...
        HANDLE left = nullptr, right = nullptr;
        if (!CreatePipe(&left, &right, &security, 0)) { Fail(State::ChildFailed, "create_stdin_pipe"); return false; }
        childStdinRead.Reset(left); parentStdinWrite.Reset(right);
        if (!CreatePipe(&left, &right, &security, 0)) { Fail(State::ChildFailed, "create_stdout_pipe"); return false; }
        parentStdoutRead.Reset(left); childStdoutWrite.Reset(right);
        if (!CreatePipe(&left, &right, &security, 0)) { Fail(State::ChildFailed, "create_stderr_pipe"); return false; }
        parentStderrRead.Reset(left); childStderrWrite.Reset(right);
        ...
        { Fail(State::ChildFailed, "parent_handle_inherit_clear"); return false; }
        ...
        return m_state.load() == State::Ready;
    }
#endif
    ...
    void UpdateWorld()
    {
        ...
        {
            if (Clock::now() >= delivery.route.deadline || !IsCanonicalSanitizedText(delivery.text)) continue;
            SessionFingerprint botSnapshot, targetSnapshot;
            {
                ...
                if (bot == m_sessions.end() || target == m_sessions.end()) continue;
                botSnapshot = bot->second; targetSnapshot = target->second;
            }
            ...
            if (!same(botSnapshot, delivery.route.botSession) || !same(targetSnapshot, delivery.route.targetSession)) continue;
            ...
            bot->Whisper(delivery.text, protocolLanguage, target->GetObjectGuid());
        }
#else
        (void)deliveries;
#endif
    ...
    std::size_t TestDrainValidDeliveries()
    {
        ...
        for (Delivery const& delivery : m_deliveries)
        {
            ...
            if (Clock::now() < delivery.route.deadline && bot != m_sessions.end() && target != m_sessions.end() &&
...
private:
        ...
        if (kind == CommandKind::Consume)
        {
            if (response.consumed)
            {
                if (response.completion.outcome == "ready" && Clock::now() < route.deadline && m_deliveries.size() < CompletionCapacity)
                    m_deliveries.push_back(Delivery{ route, response.completion.text });
                m_routes.erase(found);
            }
    ...
    void WorkerMain()
    {
        ...
        for (;;)
        {
            ...
            if (!ReadAvailable(m_stdout.Get(), stdoutBytes, stdoutEof) || !ReadAvailable(m_stderr.Get(), stderrBytes, stderrEof))
            {
            ...
            if (stdoutEof)
            {
                if (!(m_shutdownAcknowledged && m_shutdownRequested)) Fail(State::ChildFailed, "stdout_eof");
                break;
            }
            if (WaitForSingleObject(m_process.Get(), 0) == WAIT_OBJECT_0)
            {
                if (!(m_shutdownAcknowledged && m_shutdownRequested)) Fail(State::ChildFailed, "child_exit_or_abort");
                break;
            }
            if (m_forceStop.load()) break;
            if (m_shutdownRequested && Clock::now() >= m_shutdownDeadline) break;
            ...
            if (ready && pendingKind == CommandKind::None)
            {
                ...
                if (ChooseCommand(command, pendingKind, pendingRequest))
                {
                    ...
                    wire.push_back('\n');
                    if (wire.size() > StdoutNdjsonLineCapBytes || !WriteAll(m_stdin.Get(), wire))
                    { Fail(State::ChildFailed, "stdin_write_failed"); break; }
                }
            }
            std::unique_lock<std::mutex> lock(m_mutex);
        ...
        if (!m_shutdownAcknowledged && m_process.Valid() && WaitForSingleObject(m_process.Get(), 0) == WAIT_TIMEOUT)
...
private:
    ...
    std::atomic<bool> m_shutdownSent{ false };
    std::atomic<bool> m_shutdownAcknowledged{ false };
