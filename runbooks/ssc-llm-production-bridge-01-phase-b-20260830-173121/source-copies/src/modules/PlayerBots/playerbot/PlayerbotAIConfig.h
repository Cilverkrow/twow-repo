...
public:
    ...
    // Fail-closed external bridge.  The CLI path, model, language and actions
    // are contract pins rather than operator-selectable settings.
    bool externalLLMBridgeEnabled = false;
    std::string externalLLMBridgeNodeExecutable;
    std::string externalLLMBridgePackageRoot;
