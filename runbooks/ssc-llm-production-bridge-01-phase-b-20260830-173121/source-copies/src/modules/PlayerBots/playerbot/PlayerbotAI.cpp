...
#include "ExternalLLMBridgeService.h"
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket& packet)
{
    ...
    case SMSG_MESSAGECHAT: // do not react to self or if not ready to reply
    {
...
#endif
        {
            ...
            bool isExternalBridgeChat = sPlayerbotAIConfig.externalLLMBridgeEnabled &&
                bot->GetGUIDLow() == ExternalLLMBridgeService::BotGuid && msgtype == CHAT_MSG_WHISPER;
            ...
            if ((isAiChat || isExternalBridgeChat) && (lang == LANG_ADDON || message.find("d:") == 0))
                return;
            ...
            if (guid1 != bot->GetObjectGuid()) // do not reply to self
            {
                ...
                // Pinned external whispers bypass only the random ordinary
                // reply-rate filters. The value-only admission gate in
                // ChatReplyDo still revalidates the real target and route.
                if ((!isAiChat && !isExternalBridgeChat) || isFromFreeBot)
                {
                ...
                QueueChatResponse(msgtype, guid1, ObjectGuid(), message, chanName, name, isAiChat || isExternalBridgeChat);
