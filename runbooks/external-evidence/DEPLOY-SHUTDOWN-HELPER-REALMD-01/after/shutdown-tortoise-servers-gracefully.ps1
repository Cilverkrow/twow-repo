...
public static class TortoiseConsoleControl
{
    ...
    public static bool WaitForProcessExit(IntPtr processHandle, uint milliseconds)
    {
...
function Invoke-InValidatedConsole {
    ...
    try {
        ...
        $titleMatches = $false
        foreach ($allowedTitle in $AllowedWindowTitles) {
            if ([string]::Equals($actualTitle, $allowedTitle, [System.StringComparison]::Ordinal)) {
                $titleMatches = $true
                break
            }
        }
        ...
        if (-not $titleMatches) {
            $expectedTitles = "'" + ($AllowedWindowTitles -join "', '") + "'"
            throw "Unerwarteter Konsolentitel fuer PID $($ServerProcess.Id): '$actualTitle'. Erlaubt: $expectedTitles."
        }
...
try {
    ...
    if ($Action -eq 'World') {
        ...
        if ($null -eq $worldProcess) {
            Write-Output '[INFO] Worldserver laeuft bereits nicht mehr.'
            exit 0
        }

        $worldProcessHandle = [IntPtr]::Zero
        try {
            $worldProcessHandle = [TortoiseConsoleControl]::OpenProcessForWaitAndQuery([uint32]$worldProcess.Id)

            $validatedTitle = Invoke-InValidatedConsole `
                -ServerProcess $worldProcess `
                -AllowedWindowTitles @($WorldWindowTitle, ('Administrator:  ' + $WorldWindowTitle)) `
                -Operation {
            ...
            $worldWaitMilliseconds = [uint32]($WorldExitTimeoutSeconds * 1000)
            if (-not [TortoiseConsoleControl]::WaitForProcessExit($worldProcessHandle, $worldWaitMilliseconds)) {
                throw "mangosd.exe laeuft nach $WorldExitTimeoutSeconds Sekunden noch. Es wird kein erzwungener Shutdown ausgefuehrt."
            }

            $worldExitCode = [TortoiseConsoleControl]::GetProcessExitCode($worldProcessHandle)
            if ($worldExitCode -ne 0) {
                throw "mangosd.exe wurde mit dem unerwarteten Exitcode $worldExitCode beendet."
            }
        }
        finally {
            if ($worldProcessHandle -ne [IntPtr]::Zero) {
                [TortoiseConsoleControl]::CloseProcessHandle($worldProcessHandle)
                $worldProcessHandle = [IntPtr]::Zero
            }
        }
    ...
    if ([string]::IsNullOrWhiteSpace($RealmBatchWindowTitle)) {
        throw 'Der zweite erlaubte Realmd-Konsolentitel wurde nicht angegeben.'
    }

    if ([string]::Equals($RealmWindowTitle, $RealmBatchWindowTitle, [System.StringComparison]::Ordinal)) {
        throw 'Die beiden erlaubten Realmd-Konsolentitel muessen verschieden sein.'
    }
    ...
    $realmProcessHandle = [IntPtr]::Zero
    try {
        $realmProcessHandle = [TortoiseConsoleControl]::OpenProcessForWaitAndQuery([uint32]$realmProcess.Id)

        $validatedTitle = Invoke-InValidatedConsole `
            -ServerProcess $realmProcess `
            -AllowedWindowTitles @($RealmWindowTitle, $RealmBatchWindowTitle) `
            -Operation {
        ...
        $realmWaitMilliseconds = [uint32]($RealmExitTimeoutSeconds * 1000)
        if (-not [TortoiseConsoleControl]::WaitForProcessExit($realmProcessHandle, $realmWaitMilliseconds)) {
            throw "realmd.exe laeuft nach $RealmExitTimeoutSeconds Sekunden noch. Es wird kein erzwungenes Beenden ausgefuehrt."
        }

        $realmExitCode = [TortoiseConsoleControl]::GetProcessExitCode($realmProcessHandle)
        if ($realmExitCode -ne 0) {
            throw "realmd.exe wurde mit dem unerwarteten Exitcode $realmExitCode beendet."
        }
    }
    finally {
        if ($realmProcessHandle -ne [IntPtr]::Zero) {
            [TortoiseConsoleControl]::CloseProcessHandle($realmProcessHandle)
            $realmProcessHandle = [IntPtr]::Zero
        }
    }
