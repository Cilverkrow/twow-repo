...
"%POWERSHELL_EXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%SHUTDOWN_HELPER%" -Action Realm -ServerDirectory "%~dp0." -RealmWindowTitle "realmd" -RealmBatchWindowTitle "realmd - %~dp0start-realmd.bat" -RealmExitTimeoutSeconds 30
if errorlevel 1 (
    echo.
...
"%MYSQLADMIN%" --host=127.0.0.1 --port=3307 --protocol=tcp --user=root --skip-ssl --silent ping >nul 2>&1
if errorlevel 1 (
    "%POWERSHELL_EXE%" -NoLogo -NoProfile -Command "if (Get-Process -Name mysqld,mariadbd -ErrorAction SilentlyContinue) { exit 0 } else { exit 1 }"
    if errorlevel 1 (
        echo [INFO] MariaDB laeuft bereits nicht mehr.
        goto :complete
    )

    echo [FEHLER] Der MariaDB-Prozess laeuft, antwortet aber nicht auf Port 3307.
    echo          MariaDB wird nicht erzwungen beendet.
    goto :abort
)

"%MYSQLADMIN%" --host=127.0.0.1 --port=3307 --protocol=tcp --user=root --skip-ssl shutdown
if errorlevel 1 (
    echo [FEHLER] MariaDB hat den kontrollierten Shutdown abgelehnt.
    goto :abort
)

"%POWERSHELL_EXE%" -NoLogo -NoProfile -Command "$limit = [DateTime]::UtcNow.AddSeconds(30); do { $running = Get-Process -Name mysqld,mariadbd -ErrorAction SilentlyContinue; if (-not $running) { exit 0 }; Start-Sleep -Milliseconds 250 } while ([DateTime]::UtcNow -lt $limit); exit 1"
if errorlevel 1 (
