...
test('exactly two sentence-terminator runs are accepted without whitespace', () => {
  assert.equal(sanitizeAssistantText('Eins.Zwei!', runtime.config), 'Eins.Zwei!');
  assert.equal(sanitizeAssistantText('Wirklich?!Ja...', runtime.config), 'Wirklich?!Ja...');
});

test('three sentence-terminator runs without spaces are rejected', () => {
  assert.throws(
    () => sanitizeAssistantText('Eins.Zwei!Drei?', runtime.config),
